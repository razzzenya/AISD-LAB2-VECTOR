...
public:
	...
	TemplateVector(const TemplateVector& source)
	{
		size = source.size;
		eps = source.eps;
		array = new T[size];
	...
	T operator * (const TemplateVector& v) const
	{
		...
		for (size_t i = 0; i < size; ++i)
		{
			multiplication += array[i] * v.array[i];
		}
	...
	TemplateVector radius_vector(TemplateVector& v)
	{
		...
		//cout << length_a << " , " << length_b << endl; // просто печать проверки
...
public:
	...
	TemplateVector(const TemplateVector<complex<T>>& source)
	{
		size = source.size;
		eps = source.eps;
		array = new complex<T>[size];
	...
	{
		complex<T> coordinate = (array[index] + (v.array[index] * k)) / (1.0 + k);
		return coordinate;
	}
